...
class YouTokenToMeExtractor:
    ...
    def extract(self) -> Tuple[Dict[str, int], List[Tuple]]:
        ...
        # Invert key and value for vocab.txt
        vocab = dict(zip(vocab.values(), vocab.keys()))
        return vocab, merges


if __name__ == "__main__":
    parser = ArgumentParser("SentencePiece vocab.txt extractor")
    parser.add_argument(
        "--provider",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="SentencePiece model to extract vocab.txt from."
    )
    parser.add_argument(
        "--vocab.txt-output-path",
        type=str,
        required=True,
        help="Path where the vocab.txt.json file will be extracted",
    )
    parser.add_argument(
        "--merges-output-path",
        type=str,
